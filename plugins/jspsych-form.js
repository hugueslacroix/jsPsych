...
jsPsych.plugins["form"] = (function() {
    ...
    plugin.trial = function(display_element, trial) {
        ...
        function end_trial() {
            ...
            validated = end_trial_parse_schema(trial.ref.questions);
                ...
                switch (type) {
                    ...
                    case 'tab':
                        for (var l in question.questions) {
                            if(!end_trial_parse_schema(question.questions[l], i))
                              return;
                        }
                    ...
                    default:
                        ...
                        if (question.required != "" && (!value || value["Answer Received"] == "")) {
                            if(question.parent_id.search('tab-panel') != -1){
                              Tab.focus(question.parent_id);
                            }
                            focus(question.id);
                            return;
                        }
    ...
    */
    ...
    Tab.focus = function(tab_id){
      var tab_container = document.getElementById(tab_id).parentElement;

      tab_container.querySelector("a.is-active").classList.remove("is-active");
      tab_container.querySelector(".mdl-tabs__panel.is-active").classList.remove("is-active");
      //new tab link
